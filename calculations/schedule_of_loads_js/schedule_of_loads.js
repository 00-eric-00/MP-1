// returns to calculations.html
function prev() {
  window.history.back();
}
// end of returns to calculations.html
// retriving calculations data from sessionStorage
...
// end of retriving calculations data from sessionStorage
...
// end of rating of circuit breaker
// table of wire
const wire_size = [
  "2.0",
  "3.5",
...
// end of table of wire
// table of allowed ampacity for the wire
const allowed_ampacity = [
  21, 27, 36, 48, 65, 89, 119, 137, 163, 186, 214, 253, 276, 317, 340, 371, 427,
  468, 529, 617,
];
// end of table of allowed ampacity for the wire
var x = 0;
var z = 0;
  ...
  for (let i = 0; i < p_aload_description.length; i++) {
    ...
    for (let j = 0; j < circuit_breaker_rating.length; j++) {
      ...
      if (ampere < Number(circuit_breaker_rating[j])) {
        ...
        // check the wire size is appropriate for the breaker
    ...
    // check if the type of load is receptacle
    if (
    ...
    // check if the type of load is convenience outlet
    if (
    ...
    // check if the load is neither receptacle or convenience outlet
    if (
